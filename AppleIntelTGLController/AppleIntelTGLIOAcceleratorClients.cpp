...
IOReturn IntelDeviceClient::doGet_config(IOAccelDeviceConfigData* output) {
  ...
  // Populate IntelHwCapsInfo dynamically from PCI config if possible
  ...
  if (controller) {
      ...
      if (pci) {
          ...
          revID = pci->configRead8(kIOPCIConfigRevisionID);
      }
  }
  caps->fRevisionID = revID;
  caps->fRevisionID_PCH = 0x00;
  caps->fDeviceID = devID;
...
IOReturn IntelDeviceClient::doGet_device_info(IOAccelDeviceInfoData* output) {
 ...
 if (pci) {
     ...
     output->deviceID = 0x9A78; // Fallback to 0x9A78
