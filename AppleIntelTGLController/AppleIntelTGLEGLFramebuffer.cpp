...
bool IntelIOFramebuffer::start(IOService* provider) {
    ...
    // Device identification (dynamically resolved from PCI config if possible)
    uint32_t devID = 0x9A78; // Fallback to user's Tiger Lake device ID
    uint32_t subSysVendor = 0x8086;
    uint32_t subSysID = 0x7270;
    uint32_t revID = 0x01;
    if (m_pciDevice) {
        devID = m_pciDevice->configRead16(kIOPCIConfigDeviceID);
        subSysVendor = m_pciDevice->configRead16(kIOPCIConfigSubSystemVendorID);
        subSysID = m_pciDevice->configRead16(kIOPCIConfigSubSystemID);
        revID = m_pciDevice->configRead8(kIOPCIConfigRevisionID);
    }
    setProperty("vendor-id", 0x8086, 32);   // Intel vendor ID
    setProperty("device-id", (unsigned long long)devID, 32);
    setProperty("revision-id", (unsigned long long)revID, 8);
    setProperty("subsystem-vendor-id", (unsigned long long)subSysVendor, 32);
    setProperty("subsystem-id", (unsigned long long)subSysID, 32);
