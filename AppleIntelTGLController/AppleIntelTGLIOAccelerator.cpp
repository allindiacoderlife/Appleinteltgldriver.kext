...
bool IntelIOAccelerator::start(IOService* provider) {
    ...
    // GPU info dynamically resolved from PCI configuration if possible
    ...
    if (controller) {
        ...
        if (pci) {
            ...
            subSysVendor = pci->configRead16(kIOPCIConfigSubSystemVendorID);
            subSysID = pci->configRead16(kIOPCIConfigSubSystemID);
        }
    }
